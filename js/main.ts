...
class InputForm {
    ...
    readonly customListFilter = document.getElementById("custom-list-filter") as HTMLInputElement;
    ...
    initFilterList(): void {
        ...
        input.customListFilter.addEventListener("awesomplete-selectcomplete",
            function (e) {
...
function sortByDuration(mediaList: MAL.Anime[]) {
    const val = (media: MAL.Anime) => media.seriesEpisodes * media.seriesEpisodesDuration;
    mediaList.sort((a, b) => val(a) - val(b));
}

export function dispDurations() {
    const list: MAL.Anime[] = debugData["list"].anime;
    sortByDuration(list);
    const nl = [];
...
function statsElement(listKind: string, stats: ReturnType<typeof calculateStats>) {
