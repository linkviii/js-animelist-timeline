...
export class ListManager {
    ...
    userAnimeCache: Map<string, MAL.AnimeList | MAL.BadUsernameError> = new Map();
    userMangaCache: Map<string, MAL.MangaList | MAL.BadUsernameError> = new Map();
    ...
    async getAnimeList(username: string) {
        const data: MAL.AnimeList | MAL.BadUsernameError = this.userAnimeCache.get(username);
        if (data) {
    ...
    async getMangaList(username: string) {
        const data: MAL.MangaList | MAL.BadUsernameError = this.userAnimeCache.get(username);
        if (data) {
            if (!(data instanceof MAL.BadUsernameError)) {
                console.info([username, "'s data loaded from cache."].join(""));
                data.cached = true;
