...
// Global data
//
type LullSorter = (a: Lull, b: Lull) => number;

const nullSorter: LullSorter = (a: Lull, b: Lull) => 0;
...
export let activeGrouper: (_: Lull) => string = null;
...
class InputForm {
    ...
    readonly sortSecondary = null as JQuery<HTMLSelectElement>;
    readonly sortSecondaryDirection = null as JQuery<HTMLInputElement>;
    readonly sortSecondaryDirectionTxt = null as JQuery<HTMLElement>;
    ...
    initListeners(): void {
        ...
        const initSorterN = (n: number) => {
            const setSort = () => {
                ...
                switch (value) {
                    ...
                    case "user-finish": activeSorts[n] = lullDateSorter; break;
                    default:
        ...
        const setGroupBy = () => {
            ...
            switch (value) {
                ...
                case "watch-year": activeGrouper = (lull: Lull) => `00:${lull.thisEnd.slice(0, 4)}`; break;
                case "lull": activeGrouper = (lull: Lull) => lullClumper(lull.lull); break;
...
const durationSorter: LullSorter = (a: Lull, b: Lull) => {
    return -(b.lull - a.lull);
};
const lullDateSorter: LullSorter = (a: Lull, b: Lull) => {
...
const tvTimelineConfig: ATL.AnimeListTimelineConfig = {
...
function finishGrouper(anime: Anime): string {
    return anime.userFinishDate.fixedDateStr;
}
function startGrouper(anime: Anime): string {
...
function renderLullsTable(lulls: Lull[], endGroups: Partial<Record<string, MAL.Media[]>>, startGroups: Record<string, MAL.Media[]>) {

    // lulls.sort((a, b) => a.lull - b.lull);
    const sorterAB = (a: Lull, b: Lull) => {
    ...
    function displayAnimes(list: Media[]) {
    ...
    function displayEnds(lull: Lull) {
        return displayAnimes(endGroups[lull.thisEnd]);
    }
    function displayStarts(lull: Lull) {
    ...
    const columns: [string, (_: Lull) => (string | HTMLElement), string][] = [
        ...
        ["Next", (lull: Lull) => lull.nextStart, "col-date"],
...
function renderActiveList() {
    ...
    const timeline = new ATL.AnimeListTimeline(animeList, tvTimelineConfig);
...
function lullClumper(span: number): string {
